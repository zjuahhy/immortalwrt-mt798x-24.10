...
get_current_sim_slot() {
    at_command="AT^SIMSWITCH?"
    for i in `seq 1 3`
    do
        sim_slot=$(at $at_port $at_command |grep "SIMSWITCH:"|cut -d ':' -f 2|awk {'printf $1'})
        if [ "$sim_slot" == "0" ]
        then
            sim_slot="1"
            break
...
get_mode()
{
    ...
    local mode_num=$(at ${at_port} ${at_command} | grep "\^SETMODE:" | sed 's/\r//g' | awk -F':' '{print $2}')
    local mode
    case "$platform" in
        "hisilicon")
            case "$mode_num" in
...
{
    #Temperature（温度）
    at_command="AT^TDCHIPTEMP?"
	response=$(at $at_port $at_command | grep "TDCHIPTEMP"|cut -d ',' -f 2 )
...
base_info()
{
    m_debug "td base info"

    #Name（名称）
    at_command="AT+CGMM"
    name=$(at $at_port $at_command | grep -A 1 CGMM|sed -n 2p)
    #Manufacturer（制造商）
    at_command="AT+CGMI"
    manufacturer=$(at $at_port $at_command | grep -A 1 CGMI|sed -n 2p)
    #Revision（固件版本）
    at_command="AT+CGMR"
    revision=$(at $at_port $at_command | grep -A 1 CGMR|sed -n 2p)
...
sim_info()
{
    ...
    #IMEI（国际移动设备识别码）
    at_command="AT+ATI"
	imei=$(at $at_port $at_command |grep "IMEI"|cut -d ' ' -f 2)

    #SIM Status（SIM状态）
    at_command="AT+CPIN?"
	sim_status=$(at $at_port $at_command |grep CPIN:| cut -d ':' -f 2|awk {'printf $1'}|tr A-Z a-z)
    ...
    #IMSI（国际移动用户识别码）
    at_command="AT+CIMI"
	imsi=$(at $at_port $at_command |sed -n '2p' )

    #ICCID（集成电路卡识别码）
    at_command="AT^ICCID?"
	iccid=$(at $at_port $at_command |  grep "ICCID:" |cut -d ' '  -f 2)
...
network_info()
{
    m_debug  "td network info"

    #Connect Status（连接状态）

    #Network Type（网络类型）
    at_command="AT^HCSQ?"
    response=$(at ${at_port} ${at_command}  |  grep "HCSQ:" | sed 's/\r//g'|grep LTE|wc -l)
